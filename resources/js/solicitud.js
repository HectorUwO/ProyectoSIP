...
let currentStep = 1;
let completedSections = new Set(); // Track which sections are fully completed
let maxReachedStep = 1; // Track the furthest step the user has reached
  ...
  requiredFields.forEach(field => {
    if (field.type === 'text' || field.type === 'email' || field.type === 'date') {
      ...
      if (!field.files || field.files.length === 0) {
        isValid = false;
        field.classList.add('error');
        errors.push(`Debe seleccionar un archivo para "${field.closest('.form__group').querySelector('.group__label').textContent.replace('*', '').trim()}"`);
      } else {
        ...
        if (field.files[0].size > maxSize) {
          isValid = false;
          field.classList.add('error');
          errors.push(`El archivo no debe exceder 10MB`);
        }
      }
...
function showStep(stepNumber) {
  ...
  let currentSection = getSectionFromStep(stepNumber);
...
// Helper function to get section number from step number
function getSectionFromStep(stepNumber) {
  if (stepNumber >= 1 && stepNumber <= 7) return 1;
  if (stepNumber === 8) return 2;
  if (stepNumber === 14) return 3;
  if (stepNumber >= 15 && stepNumber <= 16) return 4;
  if (stepNumber === 17) return 5;
  return 1;
}
...
function updateStepIndicators(currentSection) {
  document.querySelectorAll('.step__number').forEach((num, index) => {
    ...
    // Only mark as completed if the section is in the completedSections set
    if (completedSections.has(sectionNumber)) {
      // Pasos completados - verde con checkmark
      num.classList.add('step--completed');
      
      // Si el siguiente paso también está completado, agregar clase para el ::after
      const nextSectionNumber = sectionNumber + 1;
      if (completedSections.has(nextSectionNumber)) {
...
function nextStep() {
  ...
  // Adjust step numbers after removing steps 9-13
  if (currentStep === 8) {
    nextStepNumber = 14; // Skip directly to step 14 after step 8
  }
  
  // Check if we're completing a section
  const currentSection = getSectionFromStep(currentStep);
  const nextSection = getSectionFromStep(nextStepNumber);
  
  // If moving to a new section, mark the current section as completed
...
function prevStep() {
  ...
  // Adjust step numbers after removing steps 9-13
  if (currentStep === 14) {
    prevStepNumber = 8; // Skip back to step 8 from step 14
  }
...
function submitForm() {
  ...
  updateStepIndicators(getSectionFromStep(currentStep));
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 // Inicializar el estado de los pasos (use saved currentStep if available)
...
function saveFormData() {
 ...
 formData['_maxReachedStep'] = maxReachedStep;
 formData['_completedSections'] = Array.from(completedSections);
...
function loadFormData() {
 ...
 try {
  ...
  if (formData['_currentStep']) currentStep = formData['_currentStep'];
  if (formData['_maxReachedStep']) maxReachedStep = formData['_maxReachedStep'];
  if (formData['_completedSections']) {
   completedSections = new Set(formData['_completedSections']);
  }
...
function navigateToSection(sectionNumber) {
  ...
  const maxReachedSection = getSectionFromStep(maxReachedStep);
  
  // Allow navigation to any section that has been reached before
  if (sectionNumber <= maxReachedSection) {
    // Determinar el primer paso de la sección seleccionada
    let targetStep;
    switch(sectionNumber) {
      case 1: targetStep = 1; break;
      case 2: targetStep = 8; break; // Solo una pantalla de protocolo
    ...
    // Mostrar notificación de navegación solo si cambiamos de sección
    const sectionNames = [
...
function getCurrentSection() {
  ...
  if (currentStep === 14) return 3;
  if (currentStep >= 15 && currentStep <= 16) return 4;
  if (currentStep === 17) return 5;
  return 1;
}
